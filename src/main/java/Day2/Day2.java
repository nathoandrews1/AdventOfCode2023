...
public class Day2 {
                ...
                if (!line.isEmpty()) {
                    ...
                    for (int i = 0; i < games.size(); i++) {
                        ...
                        if (games.get(i).contains("red")) {
                            //Check if the current value is the end of the subset
    ...
    public static void Day2Part2(){
        ...
        boolean[] isSubset = {false};
        HashMap<String, Integer> maxedGameValues = new HashMap<>();
        try {
            Files.lines(Paths.get(filePath)).forEach(line -> {
                if (!line.isEmpty()) {
                    //Process each line here,
                    ...
                    int valuesMultiplied = gameValues.get("red") * gameValues.get("green") * gameValues.get("blue");
                    sum[0] += valuesMultiplied;
                }
            });

            //Sum all values outside the loop
            System.out.println("Sum of all values:");
            System.out.println(sum[0]);
        } catch (IOException e) {
